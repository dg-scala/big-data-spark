...
object WikipediaRanking {
  ...
  def referencesLanguage(lang: String, article: WikipediaArticle): Boolean = article.text.split(" ").contains(lang)
  ...
  def occurrencesOfLang(lang: String, rdd: RDD[WikipediaArticle]): Int =
    rdd.aggregate(0)(
      (count, article) => if (referencesLanguage(lang, article)) count + 1 else count,
      _ + _)
  ...
  def makeIndex(langs: List[String], rdd: RDD[WikipediaArticle]): RDD[(String, Iterable[WikipediaArticle])] =
    rdd
      .flatMap(article => langs.map(lang => (lang, article)))
      .filter(pair => referencesLanguage(pair._1, pair._2))
      .groupByKey()
  ...
  def rankLangsReduceByKey(langs: List[String], rdd: RDD[WikipediaArticle]): List[(String, Int)] =
    rdd
      .flatMap(wa => langs.map {
                                 lang =>
                                   if (referencesLanguage(lang, wa)) (lang, 1)
                                   else (lang, 0)
                               }
      )
      .reduceByKey(_ + _)
